...
| `product_site_inventory_policies` | `product_site_inventory_policies` | `criticality` |
...
| `purchase_order_line_versions` | `purchase_order_line_versions` | `po_supplier_id`, `unit_price_aud`, `line_value_aud`, `scenario_ids`, `scenario_types`, `critical_order_flag` |
...
`source_loads.csv` provides upstream dataset object metadata. During load, its `pipeline_run_id` is mapped to the actual ingestion run so downstream records reference governed ingestion control evidence.
