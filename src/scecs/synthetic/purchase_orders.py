...
def generate_purchase_orders(
    ...
    *,
    ...
    scenario_details: dict[str, list[Record]],
    ...
    current_po_number = ""
    current_po_id = ""
    current_supplier: Record | None = None
    ...
    for line_number in range(1, config.po_line_count + 1):
        if lines_in_current_po == 0:
            ...
            current_supplier = suppliers[(po_sequence * 37 + rng.randrange(len(suppliers))) % len(suppliers)]
        ...
        if current_supplier is None:
            raise RuntimeError("Purchase order supplier was not initialised.")
        ...
        scenarios = tuple(scenario_details.get(canonical_key, []))
        scenario_id_values = tuple(str(row["scenario_id"]) for row in scenarios)
        scenario_type_values = tuple(str(row["scenario_type"]) for row in scenarios)
        ...
        if "overdue_critical_order" in scenario_type_values:
            status = "open"
        ...
        final_ordered_quantity = ordered_quantity
        final_base_quantity = base_quantity
        final_need_date = need_date
        final_line_value = line_value
        ...
        for amendment in range(1, amendment_count + 1):
            ...
            amended_ordered_quantity = ordered_quantity
            if amendment > 1 and rng.random() < 0.5:
                amended_ordered_quantity = ordered_quantity + 1
            amended_quantity = float(amended_ordered_quantity * factor)
            amended_line_value = amended_quantity * unit_price
            purchase_order_line_versions.append(
                {
                    ...
                    "ordered_quantity": qty(amended_ordered_quantity),
                    "order_uom": order_uom,
                    ...
                    "unit_price_aud": money(unit_price),
                    "line_value_aud": money(amended_line_value),
                    "scenario_ids": ";".join(scenario_id_values),
                    "scenario_types": ";".join(scenario_type_values),
                    "critical_order_flag": "true" if "overdue_critical_order" in scenario_type_values else "false",
                }
            )
            final_ordered_quantity = amended_ordered_quantity
            final_base_quantity = amended_quantity
            final_need_date = amended_need
            final_line_value = amended_line_value

        schedule_count = rng.randint(2, 4) if "split_schedule" in scenario_type_values else 1
        quantities = _schedule_quantities(final_base_quantity, schedule_count)
        ...
        line_snapshots.append(
            LineSnapshot(
                ...
                site_id=str(site["id"]),
                order_date=date_iso(order_date),
                need_date=date_iso(final_need_date),
                expected_date=date_iso(expected_date),
                line_status=status,
                base_quantity=final_base_quantity,
                ordered_quantity=float(final_ordered_quantity),
                order_uom=order_uom,
                unit_price=unit_price,
                line_value_aud=final_line_value,
                conversion_factor=factor,
                scenario_ids=scenario_id_values,
                scenario_types=scenario_type_values,
                schedule_ids=tuple(schedule_ids),
            )
        )
        lines_in_current_po -= 1

    _reconcile_purchase_order_statuses(purchase_order_versions, line_snapshots)
    return (
        {
...
def _reconcile_purchase_order_statuses(
    purchase_order_versions: list[Record],
    line_snapshots: list[LineSnapshot],
) -> None:
    line_statuses_by_po: dict[str, set[str]] = {}
    for line in line_snapshots:
        line_statuses_by_po.setdefault(line.purchase_order_id, set()).add(line.line_status)

    for version in purchase_order_versions:
        statuses = line_statuses_by_po[str(version["purchase_order_id"])]
        if statuses & {"open", "on_hold"}:
            version["order_status"] = "on_hold" if "on_hold" in statuses and "open" not in statuses else "open"
        elif statuses == {"cancelled"}:
            version["order_status"] = "cancelled"
        else:
            version["order_status"] = "closed"
