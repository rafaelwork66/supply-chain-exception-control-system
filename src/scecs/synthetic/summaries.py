...
def render_markdown_summary(summary_path: Path) -> str:
    ...
    lines = [
        ...
        f"- Missing supplier signals: `{data.get('missing_supplier_signal_count')}`",
        f"- Missing inventory signals: `{data.get('missing_inventory_signal_count')}`",
        f"- Combined missing-signal rate: `{data.get('missing_signal_rate')}`",
        "",
        "## Target Comparison",
        "",
